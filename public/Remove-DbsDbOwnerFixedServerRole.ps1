function Remove-DbsDbOwnerFixedServerRole {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        PS C:\> Get-DbsDbAlterPermission -SqlInstance sql2017, sql2016, sql2012 | Remove-DbsDbOwnerFixedServerRole

        Revokes non-compliant alter permissions on sql2017, sql2016, sql2012

    .EXAMPLE
        PS C:\> Get-DbsDbAlterPermission -SqlInstance sql2017, sql2016, sql2012 | Out-GridView -Passthru | Remove-DbsDbOwnerFixedServerRole -Confirm:$false
    ...
    process {
        ...
        foreach ($permission in $InputObject) {
            if ($PSCmdlet.ShouldProcess($instance, "Installing $sqlfile on $instance to $Path")) {
                ...
                try {
                    # WHICH ROLE IS IT?
                    $sql = "ALTER ROLE DROP MEMBER user;"

<#
.SYNOPSIS
Invoke-DbsCheck is a SQL-centric Invoke-Pester wrapper

.DESCRIPTION
The Invoke-DbsCheck function runs Pester tests, including *.Tests.ps1 files and Pester tests in PowerShell scripts.
...
When you specify multiple Tag values, Invoke-DbsCheck runs tests that have any of the listed tags (it ORs the tags). However, when you specify TestName and Tag values, Invoke-DbsCheck runs only describe blocks that have one of the specified TestName values and one of the specified Tag values.
...
When you specify multiple ExcludeTag values, Invoke-DbsCheck omits tests that have any of the listed tags (it ORs the tags). However, when you specify TestName and ExcludeTag values, Invoke-DbsCheck omits only describe blocks that have one of the specified TestName values and one of the specified Tag values.
...
By default, Invoke-DbsCheck writes to the host program, not to the output stream (stdout).
...
The path to the exported dbadisa config file.
...
.PARAMETER AllChecks
In the unlikely event that you'd like to run all checks, specify -AllChecks. These checks still confirm to the skip settings in Get-DbsConfig.
...
.LINK
https://dbadisa.readthedocs.io/en/latest/functions/Invoke-DbsCheck/

.EXAMPLE
Invoke-DbsCheck -Tag Backup -SqlInstance sql2016

Runs all of the checks tagged Backup against the sql2016 instance

.EXAMPLE
Invoke-DbsCheck -Tag RecoveryModel -SqlInstance sql2017, sqlcluster -SqlCredential (Get-Credential sqladmin)
...
.EXAMPLE
Invoke-DbsCheck -Check Database -ExcludeCheck AutoShrink -ConfigFile \\share\repo\prod.json
...
Set-DbsConfig -Name app.sqlinstance -Value sql2016, sql2017, sql2008, sql2008\express
Set-DbsConfig -Name app.computername -Value sql2016, sql2017, sql2008

# Look at the current configs
Get-DbsConfig

# Invoke a few tests
Invoke-DbsCheck -Tags SuspectPage, LastBackup
...
.EXAMPLE
Invoke-DbsCheck -SqlInstance sql2017 -Tags SuspectPage, LastBackup -Show Summary -PassThru | Update-DbsPowerBiDataSource
...
C:\Windows\temp\dbadisa\ and then opens the PowerBi report in PowerBi Desktop

.EXAMPLE
Get-Help -Name Invoke-Pester -Examples

Want to get super deep? You can look at Invoke-Pester's example's and run them against Invoke-DbsCheck since it's a wrapper.
...
function Invoke-DbsCheck {
    ...
    param (
        ...
        [string[]]$ExcludeCheck = (Get-PSFConfigValue -FullName 'dbadisa.command.invokedbccheck.excludecheck' -Fallback @()),
        [switch]$PassThru,
        ...
        [object[]]$Database,
        [object[]]$ExcludeDatabase = (Get-PSFConfigValue -FullName 'dbadisa.command.invokedbccheck.excludedatabase' -Fallback @()),
        [string[]]$Value,
        [string]$ConfigFile,
        [object[]]$CodeCoverage = @(),
    ...
    dynamicparam {
        $config = Get-PSFConfig -Module dbadisa
    ...
    begin {
        if (Test-PSFParameterBinding -ParameterName ConfigFile) {
            ...
            $null = Import-DbsConfig -Path $ConfigFile -WarningAction SilentlyContinue -Temporary
        }

        $config = Get-PSFConfig -Module dbadisa
        foreach ($key in $PSBoundParameters.Keys | Where-Object { $_ -like "Config*" }) {
            if ($item = $config | Where-Object { "Config$($_.Name.Replace('.', ''))" -eq $key }) {
                Set-PSFConfig -Module dbadisa -Name $item.Name -Value $PSBoundParameters.$key
            }
        }
    ...
    process {
        ...
        if (-not $SqlInstance.InputObject -and -not $ComputerName.InputObject -and -not (Get-PSFConfigValue -FullName dbadisa.app.sqlinstance) -and -not (Get-PSFConfigValue -FullName dbadisa.app.computername) -and -not (Get-PSFConfigValue -FullName dbadisa.app.cluster)) {
            Stop-PSFFunction -Message "No servers set to run against. Use Get/Set-DbsConfig to setup your servers or Get-Help Invoke-DbsCheck for additional options."
            return
        }
        ...
        $globalexcludedchecks = Get-PSFConfigValue -FullName dbadisa.command.invokedbccheck.excludecheck
        $global:ChecksToExclude = $ExcludeCheck + $globalexcludedchecks
        [string[]]$Script:ExcludedDatabases = Get-PSFConfigValue -FullName dbadisa.command.invokedbccheck.excludedatabases
        ...
        try {
            ...
            foreach ($repo in $repos) {
                if ((Test-Path $repo -ErrorAction SilentlyContinue)) {
                    ...
                    Set-PSFConfig -Module dbadisa -Name global.notcontactable -Value @()

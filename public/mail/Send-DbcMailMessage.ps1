function Send-DbsMailMessage {
    ...
    Required. Resultset from Invoke-DbsCheck.
    ...
    .PARAMETER Subject
    Specifies the subject of the e-mail message. Default is "dbadisa results"
    ...
    .EXAMPLE
    Invoke-DbsCheck -SqlInstance sql2017 -Tags SuspectPage, LastBackup -OutputFormat NUnitXml -PassThru |
    Send-DbsMailMessage -To clemaire@dbatools.io -From nobody@dbadisa.io -SmtpServer smtp.ad.local
...
#>
    ...
    begin {
        ...
        if ($InputObject.FailedCount -lt (Get-DbsConfig mail.failurethreshhold).value) {
            Stop-PSFFunction -Message "Failure count of $($InputObject.FailedCount) is below configured threshold of $((Get-DbsConfig mail.failurethreshhold).value), not sending email" -Continue
        }
        ...
        if (Test-PSFParameterBinding -ParameterName Subject -Not) {
            $PSBoundParameters['Subject'] = (Get-DbsConfig -Name mail.Subject).value
        }
        if (Test-PSFParameterBinding -ParameterName To -Not) {
            $PSBoundParameters['To'] = (Get-DbsConfig -Name mail.To).value
        }
        ...
        if (Test-PSFParameterBinding -ParameterName smtpserver -Not) {
            $PSBoundParameters['smtpserver'] = (Get-DbsConfig -Name mail.smtpserver).value
        }
        if (Test-PSFParameterBinding -ParameterName from -Not) {
            $PSBoundParameters['from'] = (Get-DbsConfig -Name mail.from).value
        }

...
Describe "Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Get works" {
        It "returns correct detailed results" {
            $results = Get-dbadisa -Name KB2992080
        ...
        It "properly supports languages" {
            $results = Get-dbadisa -Pattern KB968930 -Language Japanese -Architecture x86 -Simple
            $results.Count -eq 4
            $results.Link | Select-Object -Last 1 | Should -Match jpn

            $results = Get-dbadisa -Pattern "KB2764916 Nederlands" -Simple
            $results.Title.Count -eq 1
            $results.Link | Should -Match nl
        }

        It "properly supports OS searches" {
            $results = Get-dbadisa -Pattern KB968930 -Language Japanese -Architecture x86 -OperatingSystem 'Windows XP'
            $results.Count -eq 1
            $results.SupportedProducts -eq 'Windows XP'
        }

        It "properly supports product" {
            $results = Get-dbadisa -Pattern KB2920730 -Product 'Office 2013' | Select-Object -First 1
            $results.SupportedProducts -eq 'Office 2013'

            $results = Get-dbadisa -Pattern KB2920730 -Product 'Office 2020' | Select-Object -First 1
            $null -eq $results
        }

        It "grabs the latest" {
            $results = Get-dbadisa -Pattern 2416447, 979906
            $results.Count | Should -Be 6
            $results = Get-dbadisa -Pattern 2416447
            $results.Count | Should -Be 3
            $results = Get-dbadisa -Pattern 979906
            $results.Count | Should -Be 3
            $results = Get-dbadisa -Pattern 2416447, 979906 -Latest
            $results.Count  | Should -Be 3
        }

        It "does not overwrite links" {
            $results = Get-dbadisa -Pattern "sql 2016 sp1" -Latest -Language Japanese
            $results.Link.Count | Should -Be 3
            "$($results.Link)" -match "jpn_"
            "$($results.Link)" -notmatch "kor_"

            $results = Get-dbadisa -Pattern "sql 2016 sp1" -Latest
        ...
        if ($env:USERDOMAIN -eq "BASE") {
            It "returns the proper results for -ComputerName" {
                $results = Get-dbadisa -Pattern KB4509475 -ComputerName sql2012
                $results.Title.Count -eq 1
    ...
    Context "Save works" {
        It "supports multiple saves" {
            $results = Save-dbadisa -Path C:\temp -Name KB2992080, KB2994397
            $results[0].Name -match 'aspnet'
            $results | Remove-Item -Confirm:$false
        }
        It "downloads a small update" {
            $results = Save-dbadisa -Name KB2992080 -Path C:\temp
            $results.Name -match 'aspnet'
            Get-ChildItem -Path $results.FullName | Should -Not -Be $null
            $results | Remove-Item -Confirm:$false
        }
        It "supports piping" {
            $piperesults = Get-dbadisa -Name KB2992080 | Select-Object -First 1 | Save-dbadisa -Path C:\temp
            $piperesults.Name -match 'aspnet'
        }
        It "does not overwrite" {
            $results = Get-dbadisa -Name KB2992080 | Select-Object -First 1 | Save-dbadisa -Path C:\temp
            $results.LastWriteTime -eq $piperesults.LastWriteTime
        }
        It "does overwrite" {
            $results = Get-dbadisa -Name KB2992080 | Select-Object -First 1 | Save-dbadisa -Path C:\temp -AllowClobber
